...
public class Plateau {
	...
	public ArrayList<Coup> listeCoup(Couleur couleur) {
		...
		ArrayList<Coup> result = new ArrayList<>();
		for (int x=0;x<7;x++) {
			for (int y=5;y>-1;y--) {
				if (tableau[y][x] == VIDE) {
	...
	public Coup IA_Decision(Couleur couleur,int profondeur, Grille frame) {
		...
		ArrayList<Coup> bonCoup = new ArrayList<>();
		int score;
		int valMax = - 1000;
		int i=0;
		...
		for (Coup coup : listeCoupPossible) {
			placerPiece(coup);
			score = min(valJoueur, profondeur);
			if (score > valMax) {
				bonCoup = new ArrayList<>();
				bonCoup.add(coup);
				valMax = score;
		...
		System.out.println(""+bonCoup.size()+" coups possibles");
		return bonCoup.get(index);

	}


	private int min(int couleur, int profondeur) {
		if (verifWin(couleur)) {
			return 1000;
		...
		for (Coup coup : listeCoup((couleur==1)? 2:1)){
			placerPiece(coup);
			score = max(couleur,profondeur-1);
			if (score < valMin) {
				valMin = score;
			}
			enleverPiece(coup);
	...
	private int max(int couleur, int profondeur) {
		if (verifWin(couleur)) {
			return 1000;
		}else if (verifWin((couleur==1)? 2:1)) {
			return -1000;
		...
		for (Coup coup : listeCoup(couleur)) {
			placerPiece(coup);
			score = min(couleur, profondeur-1);
			if (score > valMax) {
				valMax = score;
			}
			enleverPiece(coup);
			//Si �a bug chercher ici !!!
		}
		return valMax;
	}
